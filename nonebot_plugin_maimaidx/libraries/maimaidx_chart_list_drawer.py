...
class ChartInfoGroup:
    def __init__(self, name: str, charts: List[ChartInfo], limit: int, line_space: int = 50) -> None:
        self.name = name
        self.charts = charts
        self.limit = limit
...
class ChartListDrawer:
    ...
    async def draw(self):
        ...
        top = 430
        for chart_group in self.chart_groups:
            top = await self.whiledraw(chart_group.charts, top) + chart_group.line_space
